    ...
    json_string = """
{
    "document": {
        ...
        "responses": [
            ...
            {
                "response_id": "aa9ba434-367c-4e37-b66c-049c7cc0c605",
                "response_message": "Payin expired after 10 minutes",
                "response_status": "error_expired",
                "shinkansen_transaction_id": "aa5a7344-9d46-4e4f-a05e-3dbd71e373ea",
                "shinkansen_transaction_message": "see response status",
                "shinkansen_transaction_status": "error",
                "transaction_id": "aa45e0f0-b13c-436a-a67f-0077f0700b8d",
                "transaction_type": "payin"               
            },
            {
                "response_id": "bb9ba434-367c-4e37-b66c-049c7cc0c605",
                "response_message": "ok",
                "response_status": "ok",
                "shinkansen_transaction_id": "bb5a7344-9d46-4e4f-a05e-3dbd71e373ea",
                "shinkansen_transaction_message": "ok",
                "shinkansen_transaction_status": "ok",
                "transaction_id": "bb45e0f0-b13c-436a-a67f-0077f0700b8d",
                "transaction_type": "payin",
                "expected_settlement_date": "2022-10-07T14:43:55Z",
                "additional_info": {
                    "debtor": {
                        "name": "Juan Perez"
                    }
    ...
    assert len(response_message.responses) == 3
    ...
    assert isinstance(response, responses.PayoutResponse)
    response = response_message.responses[1]
    assert response.response_id == "aa9ba434-367c-4e37-b66c-049c7cc0c605"
    assert response.response_message == "Payin expired after 10 minutes"
    assert response.response_status == "error_expired"
    assert response.shinkansen_transaction_id == "aa5a7344-9d46-4e4f-a05e-3dbd71e373ea"
    assert response.shinkansen_transaction_message == "see response status"
    assert response.shinkansen_transaction_status == "error"
    assert response.transaction_id == "aa45e0f0-b13c-436a-a67f-0077f0700b8d"
    assert response.transaction_type == "payin"
    assert isinstance(response, responses.PayinResponse)
    response = response_message.responses[2]
    assert response.response_id == "bb9ba434-367c-4e37-b66c-049c7cc0c605"
    assert response.response_message == "ok"
    assert response.response_status == "ok"
    assert response.shinkansen_transaction_id == "bb5a7344-9d46-4e4f-a05e-3dbd71e373ea"
    assert response.shinkansen_transaction_message == "ok"
    assert response.shinkansen_transaction_status == "ok"
    assert response.transaction_id == "bb45e0f0-b13c-436a-a67f-0077f0700b8d"
    assert response.transaction_type == "payin"
    assert response.expected_settlement_date == "2022-10-07T14:43:55Z"
    assert response.additional_info["debtor"]["name"] == "Juan Perez"
    assert isinstance(response, responses.PayinResponse)
...
def sample_response_signature_certs_and_receiver():
    ...
    sample_shinkansen_certificates = [
        jws.certificate_from_pem_bytes(b"""-----BEGIN CERTIFICATE-----
