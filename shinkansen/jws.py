...
def sign(payload: str, key: rsa.RSAPrivateKey, 
         certificate: x509.Certificate) -> str:
    ...
    if not _cert_and_key_match(certificate, key):
        raise KeyMismatch(
            f"Certificate #{certificate} and key #{key} do not match")
    jwk = JWK()
    jwk.import_from_pyca(key)    
    jws = JWS(payload)
    jws.add_signature(jwk, protected={
        "alg": "PS256",
        "b64": False,
        "crit": ["b64"],
...
def verify_detached(payload: str, detached_jws: str, 
                    certificate_whitelist: list[x509.Certificate]):
...
def main():
    ...
    parser = argparse.ArgumentParser(usage=
        "%(prog)s sign   <certificate> --key <file> [--payload <file>] \n"
    ...
    parser.add_argument("certificate", 
        help="Path to PEM certificate")
    parser.add_argument("-k", "--key", 
        help="Path to PEM private key")
    parser.add_argument("-p", "--password", 
        help="Password to decrypt PEM private key")
    parser.add_argument("-f", "--payload", 
        help="Path to payload file, utf-8 encoded (uses stdin if not specified)")
    parser.add_argument("-j", "--jws", 
        help="Path to JWS file (compact, detached, utf-8 encoded)")
    parser.add_argument("-o", "--output", 
        help="Output file (uses stdout if not specified)")
    ...
    if args.action == 'sign':
        ...
        if not args.jws:
            print("A compact detached JWS is needed for verification", 
                  file=sys.stderr)
            sys.exit(1)        
